...
export async function getSongDetails(req, res) {
    ...
    try {
        ...
        return res.status(500).json({ message: "Error al obtener la información de la canción", error: error.message });
    }
}

/**
 * Reproduce una canción por su ID y la agrega a la cola de reproducción.
 * POST /api/player/play/:songId
 */
export async function playSong(req, res) {
    const songId = req.params.songId;
    try {
        const song = await db.song.findByPk(songId, { attributes: ["id", "url_mp3"] });
        if (!song) return res.status(404).json({ message: "Canción no encontrada" });
        ...
        if (req.user) {
            await db.playlist.upsert({ userId: req.user.id, songs: JSON.stringify(currentPlaylist) });
        }

        console.log(`Reproduciendo canción: ${fullUrlMp3}`);
        return res.status(200).json({ message: `Reproduciendo canción con ID: ${song.id}`, url: fullUrlMp3, isPlaying });
    } catch (error) {
        console.error("Error al reproducir la canción:", error);
        return res.status(500).json({ message: "Error al reproducir la canción", error: error.message });
    }
}
 ...
 */
export function pauseSong(req, res) {
    isPlaying = false;
 ...
 */
export async function playNextSong(req, res) {
    if (currentPlaylist.length === 0) return res.status(400).json({ message: "La cola de reproducción está vacía" });

    currentSongIndex = (currentSongIndex + 1) % currentPlaylist.length;
    const nextSong = currentPlaylist[currentSongIndex];

    try {
        const song = await db.song.findByPk(nextSong.id, { attributes: ["url_mp3"] });
        if (!song) return res.status(404).json({ message: "Canción no encontrada" });
        ...
        return res.status(500).json({ message: "Error al reproducir la siguiente canción", error: error.message });
    }
}
 ...
 */
export async function playPreviousSong(req, res) {
    if (currentPlaylist.length === 0) return res.status(400).json({ message: "La cola de reproducción está vacía" });
    ...
    try {
        const song = await db.song.findByPk(previousSong.id, { attributes: ["url_mp3"] });
        if (!song) return res.status(404).json({ message: "Canción no encontrada" });
        ...
        return res.status(500).json({ message: "Error al reproducir la canción anterior", error: error.message });
    }
}
...
export async function getLyrics(req, res) {
    const songId = req.params.songId;
    try {
        const song = await db.song.findByPk(songId, { attributes: ["lyrics"] });
        if (!song) return res.status(404).json({ message: "Canción no encontrada" });
        if (!song.lyrics) return res.status(404).json({ message: "Letra no encontrada para esta canción" });

        return res.status(200).json({ lyrics: song.lyrics });
    } catch (error) {
        console.error("Error al obtener la letra:", error);
        return res.status(500).json({ message: "Error al obtener la letra", error: error.message });
    }
}
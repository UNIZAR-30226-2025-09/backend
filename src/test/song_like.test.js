...
describe("Rutas de song_like", () => {
    ...
    describe("Pruebas de integración", () => {
        ...
        it("Debería agregar la canción a la playlist de 'Me Gusta' al dar like", async () => {
            // Eliminar cualquier like previo
            await db.song_like.destroy({
                where: { user_id: userId }
            });

            // Dar like a la canción
            await request(BASE_URL)
                .post(`/api/song_like/${songId}/like`)
                .set('Authorization', `Bearer ${sharedToken}`)
                .send({ user_id: userId });

            // Obtener la playlist de Me Gusta
            const playlist = await db.playlist.findOne({
                where: {
                    user_id: userId,
                    typeP: 'Vibra_likedSong'
            ...
            expect(playlist).not.toBeNull();
            expect(playlist.songs.some(s => s.id === songId)).toBe(true);
        });

        it("Debería quitar la canción de la playlist de 'Me Gusta' al quitar el like", async () => {
            // Dar like a la canción
            await request(BASE_URL)
                .post(`/api/song_like/${songId}/like`)
                .set('Authorization', `Bearer ${sharedToken}`)
            ...
            expect(playlist).not.toBeNull();
            // Verificar que la canción ya no está en la playlist
            expect(playlist.songs.some(s => s.id === songId)).toBe(false);
        });
    });
});